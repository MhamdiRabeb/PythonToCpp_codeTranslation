...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
    ...
    for(int i=0; i<grid.size(); i++){
      //vector <float> row (grid.size(), vector <float> (grid[0].size(),0));
  ...
  float blurring) 
{
  ...
  double new_i = 0;
  double new_j = 0;
  float beliefs_val = 0.0;
  int  height = beliefs.size();
  int  width = beliefs[0].size();
  
  // Calculate new probabilities based on movements
  for (int row=0; row<beliefs.size();row++){
  	for (int cell=0; cell<beliefs[0].size();cell++){
      	beliefs_val = beliefs[row][cell]; 
    	if(row + dy < 0) {
          	new_i = (row + dy ) + width;
        }
        else {
        ...
        if(cell + dx < 0) {
          	new_j = (cell + dx ) + height;
        }
        else {
          	new_j = (cell + dx ) % height;
        }
      	newGrid[new_i][new_j] += beliefs_val;
  	}
  }  
	...
	float p_miss) 
{
    ...
    bool hit;
    double s = 0; 
  // calculate updated beliefs grid
